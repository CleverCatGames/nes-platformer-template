    ...
    def load(cls, fname):
        ...
        def get_properties(properties_root, fd=fd):
            ...
            for prop in properties_root.findall("property"):
                ...
                prop_type = prop.attrib.get("propertytype")
                if not value:
                    value = prop.text
                ...
                if type_ == "bool":
                    ...
                    value = get_properties(prop.find("properties"))
        ...
        def find_tileset_by_gid(gid, tilesets):
            found = None
            for ts in tilesets:
                if ts.firstgid > gid:
                    break
                found = ts
            if not found:
                raise Exception ("No tileset found for tile")
            return found

        def get_object(object_root, tilesets=[], obj=None):
            def apply_tileset(obj, ts):
                gid = obj.gid - ts.firstgid
                if gid in ts.objects:
                    obj.tile = ts.objects[gid]
                    props = obj.tile.properties.copy()
                    props.update(obj.properties)
                    obj.properties = props

            template = object_root.attrib.get("template")
            if template:
                source = os.path.abspath(os.path.join(fd, template))
                od = os.path.dirname(source)
                template_root = ET.parse(source).getroot()
                for child in template_root:
                    if child.tag == "tileset":
                        tileset = get_tileset(child, od)
                    elif child.tag == "object":
                        obj = get_object(child, [tileset], obj)
                        #apply_tileset(obj, tileset)

            if not obj:
                obj = Object("", "", 0, 0) # empty object

            obj.id = object_root.attrib.get("id", obj.id)

            obj.name = object_root.attrib.get("name", obj.name)
            obj.type = object_root.attrib.get("type", obj.type)
            obj.x = int(object_root.attrib.get("x", obj.x))
            obj.y = int(object_root.attrib.get("y", obj.y))
            obj.width = int(float(object_root.attrib.get("width", obj.width)))
            obj.height = int(float(object_root.attrib.get("height", obj.height)))
            obj.rotation = float(object_root.attrib.get("rotation", obj.rotation))
            obj.visible = bool(int(object_root.attrib.get("visible", obj.visible)))
            gid = object_root.attrib.get("gid", obj.gid)
            if gid is not None:
                gid = int(gid)
                obj.fliph = (gid & 0x80000000) != 0
                obj.flipv = (gid & 0x40000000) != 0
                obj.gid = gid & 0x0FFFFFFF
                apply_tileset(obj, find_tileset_by_gid(obj.gid, tilesets))

            for ochild in object_root:
                if ochild.tag == "properties":
                    obj.properties.update(get_properties(ochild))
                    ...
                    for coord in s.split():
                        ...
                        for n in coord.split(','):
                            if n.isdigit():
                                ...
                                pos.append(float(n))
                        obj.polyline.append(tuple(pos))
                elif ochild.tag == "text":
                    ttext = ochild.text
                    tfontfamily = ochild.attrib.get("fontfamily")
                    tpixelsize = int(ochild.attrib.get("pixelsize", 0))
                    twrap = bool(int(ochild.attrib.get("wrap")))
                    tcolor = ochild.attrib.get("color")
                    if tcolor:
                        tcolor = Color(tcolor)
                    tbold = bool(int(ochild.attrib.get("bold", 0)))
                    titalic = bool(int(ochild.attrib.get("italic", 0)))
                    tunderline = bool(int(ochild.attrib.get("underline", 0)))
                    tstrikeout = bool(int(ochild.attrib.get("strikeout", 0)))
                    tkerning = bool(int(ochild.attrib.get("kerning", 0)))
                    thalign = ochild.attrib.get("halign")
                    tvalign = ochild.attrib.get("valign")
                    obj.text = Text(ttext, tfontfamily, tpixelsize,
        ...
        def get_objectgroup(layer_root):
            ...
            for ogchild in layer_root:
                if ogchild.tag == "properties":
                    properties.update(get_properties(ogchild))
                elif ogchild.tag == "object":
                    objects.append(get_object(ogchild, self.tilesets))
        ...
        def get_imagelayer(layer_root):
            ...
            for child in layer_root:
                if child.tag == "properties":
                    properties.update(get_properties(child))
                elif child.tag == "image":
                    image = get_image(child)

            return ImageLayer(name, x, y, opacity, visible, properties, image)

        def get_tileset(troot, td):
            firstgid = int(troot.attrib.get("firstgid"))
            source = troot.attrib.get("source")

            if source is not None:
                source = os.path.abspath(os.path.join(td, source))
            ...
            for tchild in troot:
                if tchild.tag == "tileoffset":
                    ...
                    properties.update(get_properties(tchild))
                    ...
                    for terrain in tchild.findall("terrain"):
                        ...
                        for trchild in terrain:
                            if trchild.tag == "properties":
                                trproperties.update(get_properties(
                                    trchild))
                    ...
                    for tichild in tchild:
                        if tichild.tag == "properties":
                            tiproperties.update(get_properties(tichild))
                    ...
                    tiles.append(Tile(tid, titerrain, tiprobability,
                                      ...
                                      ttype))
            return Tileset(firstgid, name, tilewidth,
        ...
        for child in root:
            if child.tag == "properties":
                self.properties.update(get_properties(child))
            elif child.tag == "tileset":
                tileset = get_tileset(child, fd)
...
class Tileset(object):
    ...
    def __init__(self, firstgid, name, tilewidth, tileheight, source=None,
        ...
        self.tiles = tiles if tiles else []
        self.objects = dict((t.id, t) for t in tiles)
